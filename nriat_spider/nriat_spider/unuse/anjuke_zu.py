...
class AnjukeZuSpider(RedisSpider):
    name = 'anjuke_zu'
    allowed_domains = ['anjuke.com']
    start_urls = ['https://m.anjuke.com']
    redis_key = "anjuke_zu:start_url"
    ...
    def seed_requests(self, response):
        ...
        for i in f:
            ...
            pinyin_city = data[3]
            headers = self.get_headers(1)
            url = "https://{}.zu.anjuke.com/".format(id_city)
    ...
    def get_area(self, response):
        ...
        youxiao = re.search('(elems-l|zu-itemmod)', response.text)
        if youxiao:
            headers = self.get_headers(1)
            # 获得区域
            if not area_name and "elems-l" in response.text:
                try:
                    area_list = response.css(".sub-items.sub-level1").xpath("./a")
                    if not area_list:
                ...
                for i in area_list:
                    ...
                    if area_name and "周边" not in area_name and "全部" not in area_name:
                        meta = {"city": city, "id_city": id_city, "pinyin_city": pinyin_city, "area_name": area_name,
                                "area_url": area_url}
                        yield scrapy.Request(url=area_url, method="GET", callback=self.get_area, headers=headers,meta=meta)
            else:
                # 分页
                url_next = response.css(".aNxt").xpath("./@href").get()
                if url_next and page_num < 50:#是不是没问题
                    meta = {"city": city, "id_city": id_city, "pinyin_city": pinyin_city, "area_name": area_name,
                            "area_url": area_url, "page_num": page_num+1}
                    yield scrapy.Request(url=url_next, method="GET", callback=self.get_area, headers=headers,meta=meta)
                ##解析房屋
                goods_list = response.xpath("//div[@class='zu-itemmod' and contains(@_soj,'Filter')]")
                for i in goods_list:
                    house_url = i.xpath("./@link").get("")
                    house_id = ""
                    match_id = re.search("/(\d+)\?", house_url)
                    if match_id:
